...
function getCachedRTF(locString, opts = {}) {
    const _a = opts,
        { base } = _a,
        cacheKeyOpts = __objRest(_a, ['base'])
...
function supportsFastNumbers(loc) {
    if (loc.numberingSystem && loc.numberingSystem !== 'latn') {
        ...
        return (
            ...
            new Intl.DateTimeFormat(loc.intl).resolvedOptions()
...
var PolyNumberFormatter = class {
    constructor(intl, forceSimple, opts) {
        ...
        const _a = opts,
            { padTo, floor } = _a,
            otherOpts = __objRest(_a, ['padTo', 'floor'])
        ...
        if (this.originalZone) {
            return parts.map((part) => {
                if (part.type === 'timeZoneName') {
                    const offsetName = this.originalZone.offsetName(
                        this.dt.ts,
                        {
...
var Locale = class _Locale {
    ...
    redefaultToEN(alts = {}) {
        return this.clone(
            __spreadProps(__spreadValues({}, alts), {
                defaultToEN: true,
            }),
        )
    }
...
function normalizeZone(input, defaultZone2) {
    ...
    if (isUndefined(input) || input === null) {
        ...
        if (lowered === 'default') return defaultZone2
            ...
            return (
                FixedOffsetZone.parseSpecifier(lowered) ||
                IANAZone.create(input)
            )
...
function digitRegex({ numberingSystem }, append = '') {
    ...
    if (!digitRegexCache[ns][append]) {
        digitRegexCache[ns][append] = new RegExp(
            `${numberingSystems[ns]}${append}`,
        )
    }
...
function firstWeekOffset(year, minDaysInFirstWeek, startOfWeek) {
    const fwdlw = isoWeekdayToLocal(
        dayOfWeek(year, 1, minDaysInFirstWeek),
        startOfWeek,
    )
...
var Formatter = class _Formatter {
    ...
    formatWithSystemDefault(dt, opts) {
        ...
        const df = this.systemLoc.dtFormatter(
            dt,
            __spreadValues(__spreadValues({}, this.opts), opts),
        )
        return df.format()
    }
    dtFormatter(dt, opts = {}) {
        ...
        const knownEnglish = this.loc.listingMode() === 'en',
            ...
            tokenToString = (token) => {
                switch (token) {
                    ...
                    case 'ii':
                        return this.num(
                            dt.localWeekYear.toString().slice(-2),
                            2,
                        )
                    case 'iiii':
        ...
        extractors
            .reduce(
                ([mergedVals, mergedZone, cursor], ex) => {
                    ...
                    return [
                        __spreadValues(__spreadValues({}, mergedVals), val),
                        zone || mergedZone,
                        next,
                    ]
                },
...
function clone(dur, alts, clear = false) {
    const conf = {
        values: clear
            ? alts.values
            : __spreadValues(__spreadValues({}, dur.values), alts.values || {}),
...
var Duration = class _Duration {
    ...
    toFormat(fmt, opts = {}) {
        ...
        return this.isValid
            ? Formatter.create(this.loc, fmtOpts).formatDurationFromString(
                  this,
                  fmt,
              )
            : INVALID
    ...
    set(values) {
        if (!this.isValid) return this
        const mixed = __spreadValues(
            __spreadValues({}, this.values),
...
var Interval = class _Interval {
    ...
    divideEqually(numberOfParts) {
        if (!this.isValid) return []
        return this.splitBy(this.length() / numberOfParts).slice(
            0,
            numberOfParts,
        )
    }
            ...
            .reduce(
                ([sofar, current], item) => {
                    if (!current) {
                        ...
                        current.overlaps(item) ||
                        current.abutsStart(item)
                    ) {
...
var Info = class {
    ...
    static weekdays(
        ...
        return (
            locObj || Locale.create(locale, numberingSystem, null)
        ).weekdays(length)
    }
    /**
...
function diff_default(earlier, later, units, opts) {
    ...
    if (lowerOrderUnits.length === 0) {
        ...
        if (highWater !== cursor) {
            results[lowestOrder] =
                (results[lowestOrder] || 0) +
                remainingMillis / (highWater - cursor)
        }
    }
...
function tokenForPart(part, formatOpts, resolvedOpts) {
    ...
    if (type === 'hour') {
        if (formatOpts.hour12 != null) {
            ...
            if (
                formatOpts.hourCycle === 'h11' ||
                formatOpts.hourCycle === 'h12'
            ) {
                actualType = 'hour12'
            } else {
                actualType = 'hour24'
...
function clone2(inst, alts) {
    ...
    return new DateTime(
        __spreadProps(__spreadValues(__spreadValues({}, current), alts), {
            old: current,
        }),
    )
}
...
function adjustTime(inst, dur) {
    const oPre = inst.o,
        ...
        month =
            inst.c.month +
            Math.trunc(dur.months) +
            Math.trunc(dur.quarters) * 3,
...
var DateTime = class _DateTime {
    ...
    set(values) {
        ...
        if (settingWeekStuff) {
            ...
            mixed = ordinalToGregorian(
                __spreadValues(
                    __spreadValues({}, gregorianToOrdinal(this.c)),
                    normalized,
                ),
            )
        } else {
            mixed = __spreadValues(
                __spreadValues({}, this.toObject()),
                normalized,
            )
            if (isUndefined(normalized.day)) {
                mixed.day = Math.min(
                    daysInMonth(mixed.year, mixed.month),
                    mixed.day,
                )
            }
        }
   ...
   */
    ...
    toFormat(fmt, opts = {}) {
        return this.isValid
            ? Formatter.create(
                  this.loc.redefaultToEN(opts),
              ).formatDateTimeFromString(this, fmt)
            : INVALID3
    }
    /**
    ...
    toLocaleString(formatOpts = DATE_SHORT, opts = {}) {
        return this.isValid
            ? Formatter.create(this.loc.clone(opts), formatOpts).formatDateTime(
                  this,
              )
            : INVALID3
    }
    ...
    toLocaleParts(opts = {}) {
        return this.isValid
            ? Formatter.create(this.loc.clone(opts), opts).formatDateTimeParts(
                  this,
              )
            : []
    }
    ...
    static fromFormatParser(text, formatParser, opts = {}) {
        ...
        const { result, zone, specificOffset, invalidReason } =
