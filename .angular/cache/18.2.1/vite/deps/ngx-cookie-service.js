import { DOCUMENT, isPlatformBrowser } from './chunk-6YJC62YB.js'
import {
    Inject,
    Injectable,
    PLATFORM_ID,
...
var _CookieService = class _CookieService {
    ...
    get(name) {
        if (this.documentIsAccessible && this.check(name)) {
            ...
            return result[1]
                ? _CookieService.safeDecodeURIComponent(result[1])
                : ''
        } else {
            return ''
        }
    ...
    getAll() {
        ...
        if (document.cookie && document.cookie !== '') {
            document.cookie.split(';').forEach((currentCookie) => {
                ...
                cookies[
                    _CookieService.safeDecodeURIComponent(
                        cookieName.replace(/^ /, ''),
                    )
                ] = _CookieService.safeDecodeURIComponent(cookieValue)
            })
        }
        return cookies
    }
    set(
        ...
        if (options.secure === false && options.sameSite === 'None') {
            options.secure = true
            console.warn(
                `[ngx-cookie-service] Cookie ${name} was forced with secure flag because sameSite=None.More details : https://github.com/stevermeister/ngx-cookie-service/issues/86#issuecomment-597720130`,
            )
        }
        if (options.secure) {
            cookieString += 'secure;'
